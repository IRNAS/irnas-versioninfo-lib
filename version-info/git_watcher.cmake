...
CHECK_OPTIONAL_VARIABLE(GIT_WORKING_DIR "${CMAKE_BINARY_DIR}")
...
function(GetGitState _working_dir)
    ...
    # From output of git describe extract version
    rungitcommand(describe --tags --always --long --dirty=+)
    set(GIT_VERSION "${output}")

    string(REGEX MATCH "^v[0-9]+\\.[0-9]+\\.[0-9]+-[0-9]+-g(.*)$"
               PROPER_GIT_DESCRIBE "${GIT_VERSION}")

    if(PROPER_GIT_DESCRIBE)
        string(REGEX REPLACE "^v([0-9]+).*$" "\\1"
            VERSION_MAJOR ${GIT_VERSION})

        string(REGEX REPLACE "^v[0-9]+\\.([0-9]+).*$" "\\1"
            VERSION_MINOR "${GIT_VERSION}")

        string(REGEX REPLACE "^v[0-9]+\\.[0-9]+\\.([0-9]+).*$" "\\1"
            VERSION_PATCH "${GIT_VERSION}")
    else()
        set(VERSION_MAJOR 0)
        set(VERSION_MINOR 0)
        set(VERSION_PATCH 0)
    ...
    set(ENV{VERSION_INFO_GIT_DESCRIBE} ${GIT_VERSION})
    set(ENV{VERSION_INFO_MAJOR} ${VERSION_MAJOR})
    set(ENV{VERSION_INFO_MINOR} ${VERSION_MINOR})
    set(ENV{VERSION_INFO_PATCH} ${VERSION_PATCH})

    string(REGEX REPLACE "^v[0-9]+\\.[0-9]+\\.[0-9]+-[0-9]+-g(.*)$" "\\1"
        VERSION_HASH "${GIT_VERSION}")

    # Check whether the git hash ends with plus sign
    if("${VERSION_HASH}" MATCHES "(\\+)$")
        set(ENV{VERSION_INFO_DIRTY} "true")
    else()
        set(ENV{VERSION_INFO_DIRTY} "false")
    endif()

    rungitcommand(rev-parse --short HEAD)
    set(ENV{VERSION_INFO_HASH} "${output}")
